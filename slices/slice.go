...
func SlicesExamples() {
	...
	fmt.Println("Remove by loop", removeByIndexLoop(x, 2)) // [10 20 40 50 60 70 80 90]
	// 2. Remove by copy (efficient but not readable)
	fmt.Println("Remove by copies", removeNewSliceByCopies(x, 2)) // [10 20 40 50 60 70 80 90]
	// 3. Remove by copy (efficient and readable)
	fmt.Println("Remove by copy", removeNewSliceByCopy(x, 2)) // [10 20 40 50 60 70 80 90]
	// 4. Remove by append (efficient and readable)
	fmt.Println("Remove by append", removeByAppend(x, 2)) // [10 20 40 50 60 70 80 90]
	// 5. Remove by slices package (efficient and readable)
	fmt.Println("Remove by slices package", removeBySlices(x, 2)) // [10 20 40 50 60 70 80 90]
